---
title: "Myocardium Analysis (Figure 6 and 7)"
author: "Clay Carey"
date: "2023-04-12"
output: pdf_document
---
...
zf_3dpi_down <- filter(zf_3dpi_markers, avg_log2FC < 0 & p_val_adj < 0.01 )

md_3dpi_up <- filter(md_3dpi_markers, avg_log2FC > 0 & p_val_adj < 0.01  )
md_3dpi_down <- filter(md_3dpi_markers, avg_log2FC < 0 & p_val_adj < 0.01 )

p4_3dpi_up <- filter(p4_3dpi_markers, avg_log2FC > 0 & p_val_adj < 0.01 )
p4_3dpi_down <- filter(p4_3dpi_markers, avg_log2FC < 0 & p_val_adj < 0.01  )

p11_3dpi_up <- filter(p11_3dpi_markers, avg_log2FC > 0 & p_val_adj < 0.01 ) 
p11_3dpi_down <- filter(p11_3dpi_markers, avg_log2FC < 0 & p_val_adj < 0.01  )


zf_3dpi_up_genes <- rownames(zf_3dpi_up)
zf_3dpi_down_genes <- rownames(zf_3dpi_down)
md_3dpi_up_genes <- rownames(md_3dpi_up)
md_3dpi_down_genes <- rownames(md_3dpi_down)
p4_3dpi_up_genes <- rownames(p4_3dpi_up)
p4_3dpi_down_genes <- rownames(p4_3dpi_down)
p11_3dpi_up_genes <- rownames(p11_3dpi_up)
p11_3dpi_down_genes <- rownames(p11_3dpi_down)
...
convert_genes_md <- function(X){
  ...
  zf2md_filtered <- filter(zf2md, type == 'ortholog_one2one' & score == 1)
...
zf_3dpi_down_conv <- convert_genes_zf(zf_3dpi_down_genes)
zf_3dpi_down_conv <- remove_mt(zf_3dpi_down_conv)
...
md_3dpi_down_conv <- convert_genes_md(md_3dpi_down_genes)
md_3dpi_down_conv <- remove_mt(md_3dpi_down_conv)
...
zf_down_list <- list( Mouse_p4 = p4_3dpi_down_genes, Zebrafish = zf_3dpi_down_conv, Mouse_p11 = p11_3dpi_down_genes)
md_down_list <- list( Mouse_p4 = p4_3dpi_down_genes, Medaka = md_3dpi_down_conv, Mouse_p11 = p11_3dpi_down_genes)
all_down_list <- list( p4 = p4_3dpi_down_genes, zebrafish = zf_3dpi_down_conv,medaka = md_3dpi_down_conv, p11 = p11_3dpi_down_genes)

ggVennDiagram(zf_up_list, show_intersect = TRUE)
ggVennDiagram(md_up_list,show_intersect = TRUE)
ggVennDiagram(all_up_list,show_intersect = TRUE)

ggVennDiagram(zf_down_list, show_intersect = TRUE)
ggVennDiagram(md_down_list,show_intersect = TRUE)
ggVennDiagram(all_down_list,show_intersect = TRUE)
```
